...
$(document).ready(function () {
  ...
  function printdir(response, previousState) {
    ...
    $.each(currentBrowsingList, function () {
      ...
      if (this.type == 'directory') {
        filelist.push('<div class="clear relative"><div data-directory="' + this.name + '" class="dirz">\
            <img class="folder-image" src="/public/img/folder.svg"><span class="item-text">' + this.name + '</span></div>\
        <div class="song-button-box">\
          <span title="Add All To Queue" class="recursiveAddDir" data-directory="' + this.name + '">\
            <svg xmlns="http://www.w3.org/2000/svg" height="12" width="12" viewBox="0 0 1280 1276"><path d="M6760 12747 c-80 -5 -440 -10 -800 -11 -701 -2 -734 -4 -943 -57 -330 -84 -569 -281 -681 -563 -103 -256 -131 -705 -92 -1466 12 -241 16 -531 16 -1232 l0 -917 -1587 -4 c-1561 -3 -1590 -3 -1703 -24 -342 -62 -530 -149 -692 -322 -158 -167 -235 -377 -244 -666 -43 -1404 -42 -1813 7 -2355 21 -235 91 -400 233 -548 275 -287 730 -389 1591 -353 1225 51 2103 53 2330 7 l60 -12 6 -1489 c6 -1559 6 -1548 49 -1780 100 -535 405 -835 933 -921 88 -14 252 -17 1162 -24 591 -4 1099 -4 1148 1 159 16 312 56 422 112 118 59 259 181 333 290 118 170 195 415 227 722 18 173 21 593 6 860 -26 444 -32 678 -34 1432 l-2 811 54 7 c30 4 781 6 1670 5 1448 -2 1625 -1 1703 14 151 28 294 87 403 168 214 159 335 367 385 666 15 85 29 393 30 627 0 105 4 242 10 305 43 533 49 1047 15 1338 -44 386 -144 644 -325 835 -131 140 -278 220 -493 270 -92 21 -98 21 -1772 24 l-1680 3 3 1608 c2 1148 0 1635 -8 1706 -49 424 -255 701 -625 841 -243 91 -633 124 -1115 92z" transform="matrix(.1 0 0 -.1 0 1276)"/></svg></span>\
          <span title="Download Directory" class="downloadDir" data-directory="' + this.name + '">\
            <svg width="14" height="14" viewBox="0 0 2048 2048" xmlns="http://www.w3.org/2000/svg"><path d="M1803 960q0 53-37 90l-651 652q-39 37-91 37-53 0-90-37l-651-652q-38-36-38-90 0-53 38-91l74-75q39-37 91-37 53 0 90 37l294 294v-704q0-52 38-90t90-38h128q52 0 90 38t38 90v704l294-294q37-37 90-37 52 0 91 37l75 75q37 39 37 91z"/></svg></span>\
          <span title="Move to main collection" class="moveAlbum" data-directory="' + this.name + '">\
            <img src="/public/img/moveAlbum.svg"></span>\
          </div></div>');
      } else {
        if (this.type == 'm3u') {
      ...
      if (lowerCase.indexOf(searchVal.toLowerCase()) !== -1) {
        if (this.type === 'directory') {
          ...
          if (programState[programState.length - 1].state === 'playlist') {
            ...
            } else if (this.metadata.aaFile) {
              filelist.push('<div data-file_location="' + this.filepath + '" class="filez"><img class="album-art-box"  data-original="/album-art/' + this.metadata.aaFile + '?token=' + MSTREAMAPI.currentServer.token + '"><span class="explorer-label-1">' + this.metadata.artist + ' - ' + this.metadata.title + '</span></div>');
            } else {
  ...
  // when you click on move a directory, move it to the main collection
  $("#filelist").on('click', '.moveAlbum', function () {
    var album = $(this).data("directory");    
    var fullPath = getDirectoryString($(this));

    MSTREAMAPI.moveAlbum(album, fullPath, function(res, err){
      if (err !== false) {
        return boilerplateFailure(res, err);        
      }
  ...
  $("#filelist").on('click', '.playlistz', function () {
    ...
    MSTREAMAPI.loadPlaylist(playlistname, function (response, error) {
      ...
      $.each(response, function (index, value) {
        ...
        } else if (value.metadata.aaFile) {
          currentBrowsingList.push({ type: 'file', name: value.metadata.artist + ' - ' + value.metadata.title, metadata: value.metadata });
          files.push('<div data-lokiid="'+value.lokiId+'" class="clear relative"><div data-lokiid="'+value.lokiId+'" data-file_location="' + value.filepath + '" class="filez left"><img class="album-art-box"  data-original="/album-art/' + value.metadata.aaFile + '?token=' + MSTREAMAPI.currentServer.token + '"><span class="explorer-label-1">' + value.metadata.artist + ' - ' + value.metadata.title + '</span></div><div class="song-button-box"><span data-lokiid="'+value.lokiId+'" class="removePlaylistSong">remove</span></div></div>');
        } else {
          currentBrowsingList.push({ type: 'file', name: value.metadata.artist + ' - ' + value.metadata.title, metadata: value.metadata });
    ...
    MSTREAMAPI.getRated(function (response, error) {
      ...
      $.each(response, function (index, value) {
        ...
        if (!value.metadata || !value.metadata.title) {
          ...
          currentBrowsingList.push({ type: 'file', name: value.metadata.artist + ' - ' + value.metadata.title, metadata: value.metadata });
          files.push('<div data-file_location="' + value.filepath + '" class="filez"><img class="album-art-box"  data-original="/album-art/' + value.metadata.aaFile + '?token=' + MSTREAMAPI.currentServer.token + '"><span class="explorer-label-1">[' + rating + '] ' + value.metadata.artist + ' - ' + value.metadata.title + '</span></div>');
        } else {
          currentBrowsingList.push({ type: 'file', name: value.metadata.artist + ' - ' + value.metadata.title, metadata: value.metadata });
  ...
  //////////////////////// Random Play
  $('.random_play_settings').on('click', function () {
    $('ul.left-nav-menu li').removeClass('selected');
    $('.random_play_settings').addClass('selected');
    resetPanel('Random Play', 'scrollBoxHeight2');
    currentBrowsingList = [];
    $('#directory_bar').hide();

    var newHtml = '<br><p>Random Play randomly generates a playlist.  Click the \'RF\' button on the bottom enable it</p><h3>Use Folders</h3><p>';
  ...
  $('#filelist').on('click', 'input[name="randomPlay-folders"]', function(){
    // Don't allow user to deselect all options
    if ($('input[name="randomPlay-folders"]:checked').length < 1) {
      $(this).prop('checked', true);
      iziToast.warning({
        title: 'Random Play requires a directory',
        position: 'topCenter',
        timeout: 3500
      });
      return;
